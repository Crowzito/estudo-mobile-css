...
export default function FuncionarioFormScreen({ navigation, route }) {
  const funcionarioAntigo = route.params || {};

  const [nome, setNome] = useState(funcionarioAntigo.nome || "");
  const [cpf, setCpf] = useState(funcionarioAntigo.cpf || "");
  const [dataDeNascimento, setDataDeNascimento] = useState(
    funcionarioAntigo.dataDeNascimento || ""
  );
  const [estadoCivil, setEstadoCivil] = useState(
    funcionarioAntigo.estadoCivil || ""
  );

  const [estadoDeNascimento, setEstadoDeNascimento] = useState(
    funcionarioAntigo.estadoDeNascimento || ""
  );
  ...
  async function salvar() {
    try {
      ...
      if (funcionarioAntigo.id) {
        funcionario.id = funcionarioAntigo.id;
        await FuncionarioService.atualizar(funcionario);
      } else {
      ...
      navigation.reset({
        index: 0,
        routes: [{ name: "FuncionarioListaScreen" }],
      });
